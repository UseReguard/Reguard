"""Deterministic agent-runtime compliance pipeline CLI.

Usage:
    PYTHONPATH=src python scripts/maintenance/compliance_pipeline.py run --repo OWNER/NAME [--sha SHA]
    PYTHONPATH=src python scripts/maintenance/compliance_pipeline.py run-gold --limit 3
    PYTHONPATH=src python scripts/maintenance/compliance_pipeline.py list-repos
    PYTHONPATH=src python scripts/maintenance/compliance_pipeline.py list-requirements
    PYTHONPATH=src python scripts/maintenance/compliance_pipeline.py recent [--requirement ID]
"""
...
def _gold_repos(db_path: Path) -> list[str]:
    ...
    try:
        cur = conn.execute(
            """
            SELECT a.full_name
            FROM agent_repository_current_status a
...
def _sha_for(db_path: Path, full_name: str) -> str | None:
    """Look up the most recent recorded SHA for the repo (heuristic:
    sha may not be in the DB — return None to trigger default head)."""
    conn = sqlite3.connect(db_path)
    try:
        cur = conn.execute(
            """
...
def _cmd_run_gold(args: argparse.Namespace) -> int:
    ...
    for full_name in gold:
        ...
        if sha is None:
            # No prior run; skip — the orchestrator will need a
            # concrete SHA to clone against.
            results.append({
